//! `identity.kind: upstream` for the WhatsApp Cloud adapter (FR-I-7).
//!
//! Port of google_chat's upstream identity strategy: instead of an
//! operator-enumerated `sender_table`, the adapter delegates sender
//! resolution to a resolver tool reached through the upstream router.
//! The resolver receives `{platform: "whatsapp", sender: <wa_id>}`
//! and returns `{sub, scopes, tenant}`; any failure rejects the
//! inbound with 401 — never a guessed principal.
//!
...
async fn upstream_identity_resolves_principal_and_dispatches_with_it() {
    ...
    assert_eq!(
        resolver_bodies[0],
        json!({ "platform": "whatsapp", "sender": UNKNOWN_WA_ID }),
...
async fn upstream_identity_rejects_with_401_when_resolver_fails() {
    ...
    let resp = post_inbound(&proc, UNKNOWN_WA_ID, "resolver will fail").await;
    assert_eq!(
        resp.status(),
        401,
