//! Twilio-RCS adapter (`kind: twilio_rcs`) — text + narration only,
//! `sender_table` identity only (mirrors WhatsApp's PR-T2 scope).
//!
//! Shares the exact same [`crate::courier::TwilioCourierClient`] and
//! [`crate::signature`] verification as [`crate::whatsapp`] — both ride
//! Twilio's single Messaging API, differing only in the `To`/`From`
//! address prefix and, per Twilio's docs, in one real simplification:
//! **RCS plain text ships via `Body` directly, no Content Template
//! required** (unlike WhatsApp, which needs one for anything outside an
//! active conversation). So there is no `deliver_template`/`category`
//! path here — `deliver` always sends free-form. Rich cards/carousels
//! (which DO need a Content Template, per Twilio's RCS docs) are out of
//! scope until there's a concrete need.
//!
//! Twilio's RCS addressing (confirmed against Twilio's "Send an RCS
//! message" docs before writing this):
//!   * `From` — `rcs:<agent-id>` (an RCS Sender/Agent id issued by
//!     Twilio when the RCS agent is provisioned, NOT a phone number).
//!   * `To` — `rcs:+<E.164>`.
//!
//! Inbound webhook shape mirrors WhatsApp's (`From`/`To`/`Body`, same
//! `x-www-form-urlencoded` + `X-Twilio-Signature` mechanism) since both
//! ride the same underlying Messaging webhook.
//!
//! Deferred to follow-up PRs: `upstream` identity mode, delivery-receipt
//! status callbacks (PR-T6), rich cards/carousels (Content Template
//! machinery, if ever needed).
...
pub const PROTOCOL: &str = "messenger:twilio_rcs";
...
pub struct TwilioRcsAdapter {
    ...
    /// API courier. Resolved independently from `auth_token` (Codex
    /// review finding on the WhatsApp adapter, #191 — the two must not
    /// be silently conflated even though they're typically the same
    /// underlying Twilio Auth Token).
    outbound_token: String,
    account_sid: String,
    /// The exact externally-visible URL Twilio POSTs to — see
    /// `TwilioWhatsAppAdapter::public_url`'s doc for why this can't be
...
async fn handle_webhook(
    State(adapter): State<Arc<TwilioRcsAdapter>>,
    headers: HeaderMap,
    body: Bytes,
) -> Response {
    ...
    if let Err(retry_after) = adapter.rate_limit.try_take() {
        record_rejection(
            ...
            TritonError::RateLimited(format!(
                "twilio_rcs adapter `{}` rate limit hit; retry in {:.2}s",
    ...
    // See whatsapp.rs's handle_webhook for why ButtonPayload is
    // preferred: a tap on rich-card suggested actions/replies (should
    // Content Templates ever be added for RCS) would arrive the same
    // way — an ordinary inbound message carrying that field.
    ...
    let Some(sender_key) = from.strip_prefix("rcs:") else {
        record_rejection(
            &adapter,
            "-",
            "-",
            TritonError::Validation("From is missing the `rcs:` prefix".into()),
        );
    ...
    match result {
        Ok(dispatch) => match render_dispatch_result(&dispatch.result) {
            ...
            Err(surface_mapper::RenderError::EmptyAfterRender) => {
                tracing::warn!(
                    tool = %tool_name,
                    "twilio_rcs surface mapper: empty surface; skipping post-back",
                );
                let provider =
                    TritonError::Provider("twilio_rcs surface mapper: empty surface".into());
        ...
        Err(e) => {
            tracing::warn!(error = %e, class = %e.class(), "twilio_rcs tool dispatch failed");
            Ok(())
        }
    }
}
...
fn log_deferrals(tool_name: &str, rendered: &RenderedMessage) {
    if rendered.deferred_buttons > 0 {
        tracing::warn!(
            ...
            "twilio_rcs surface mapper: Button components deferred (no rich-card wiring yet)",
        );
    }
    if rendered.deferred_selections > 0 {
        tracing::warn!(
            ...
            "twilio_rcs surface mapper: Selection components deferred",
        );
    }
    if rendered.deferred_forms > 0 {
        tracing::warn!(
            ...
            "twilio_rcs surface mapper: Form components deferred",
        );
    }
    if rendered.deferred_dashboards > 0 {
        tracing::warn!(
            ...
            "twilio_rcs surface mapper: Dashboard components deferred (no rasterizer wiring yet)",
        );
    }
    if rendered.truncated {
        tracing::warn!(
            ...
            "twilio_rcs surface mapper: rendered text exceeded cap; truncated",
        );
    }
}
...
async fn post_back(
    adapter: &TwilioRcsAdapter,
    principal: &Principal,
    tool_name: &str,
    to: &str,
    msg: &RenderedMessage,
) {
    let to_param = format!("rcs:{to}");
    let params = [
...
fn record_rejection(adapter: &TwilioRcsAdapter, sub: &str, tenant: &str, e: TritonError) {
...
#[async_trait]
impl OutboundCourier for TwilioRcsAdapter {
    ...
    /// Deliver an agent-initiated send. Unlike WhatsApp, RCS plain text
    /// ships via `Body` unconditionally — no service window, no
    /// category/template resolution needed.
    async fn deliver(
        &self,
        req: &OutboundRequest,
        principal: &Principal,
