...
## Rendering a kind *nobody* defines

Returning null from `componentBuilder` means *decline*, and declining falls
through to an amber debug card. That is right for an operator console and
wrong on an end-user's phone, where the correct behaviour is to render
nothing. `unknownComponentBuilder` is the seam for that decision — consulted
only after `componentBuilder` declined and the version's switch found no
rule, and returning a `Widget` (not a `Widget?`) so "render nothing" is
something a host can actually say:

```dart
A2UIRenderer(
  envelope: envelope,
  unknownComponentBuilder: (context, node) => const SizedBox.shrink(),
)
```

Leave it null to keep the debug card. Present in both version trees.

## A button that names a resource

A v0.9 `button` may carry a `resource` (`ui://…`) *beside* its tool call.
`onAction` is `(tool, args)` and has nowhere to put it, so the resource
arrives through a sibling callback carrying all three — widening `onAction`
would have broken every existing host, and routing it through
`onOpenResource` would have dropped the tool call:

```dart
A2UIRenderer(
  envelope: envelope,
  onAction: (tool, args) => invoke(tool, args),
  onResourceAction: (tool, args, resource) => openInline(resource),
)
```

Omit it and a resource-carrying button dispatches through `onAction`
exactly as it did before, resource unreachable.

## The v0.9 vocabulary

Beyond the base kinds, the v0.9 tree renders what triton#206 added:
`text.pills` (wikilink labels, with the *bare id* as the deliberate degrade
when a label was withheld), `form.fields[].placeholder` / `.default` (an
untouched form submits the agent's proposal, and only for fields that carry
one), `button.primary`, and a `report`'s inline preview
(`title`/`series`/`labels`) drawn beside — never instead of — the
`render_report` dispatch control.

Those are pinned by `test/v09_vocabulary_test.dart` against envelopes a
**real** Triton process produced: the fixtures in `test/fixtures/` are
generated by `crates/triton-tests/tests/a2ui_flutter_fixtures.rs`, which
re-produces and byte-compares them on every CI run. A wire change this
renderer has not been taught fails there rather than being silently dropped
here.
